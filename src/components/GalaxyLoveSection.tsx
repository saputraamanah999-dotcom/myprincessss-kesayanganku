 ...
 *  - Galeri tombol estetik (reset, fullscreen, musik).
...
import { Maximize2, Minimize2, RotateCcw, Music, Music2, Sparkles } from "lucide-react";
...
/*                              YOUTUBE AUDIO                                */
/* -------------------------------------------------------------------------- */
/*  Memutar lagu latar ala YouTube IFrame API.  ID lagu dapat diubah bebas.  */
const GALAXY_SONG = {
  videoId: "d4OMqGKBl6E", // lagu latar galaksi
  start: 227,
  end: 261,
};
...
function initGalaxy(
  ...
  async function ensureYouTubePlayer() {
    ...
    try {
      ...
      // create hidden player div scoped to container
      const playerHost = document.createElement("div");
      playerHost.style.position = "absolute";
      playerHost.style.width = "0";
      playerHost.style.height = "0";
      playerHost.style.opacity = "0";
      playerHost.style.pointerEvents = "none";
      playerHost.style.left = "-9999px";
      playerHost.style.top = "-9999px";
      container.appendChild(playerHost);

      ytPlayer = new window.YT.Player(playerHost, {
        height: "0",
        width: "0",
        videoId: GALAXY_SONG.videoId,
        playerVars: {
          autoplay: 0,
          start: GALAXY_SONG.start,
          end: GALAXY_SONG.end,
          controls: 0,
          loop: 1,
          playlist: GALAXY_SONG.videoId,
        },
        events: {
          onReady: () => {
            ytReady = true;
            handlers.onAudioReady?.();
          },
          onStateChange: (event: any) => {
            if (event.data === window.YT.PlayerState.ENDED) {
              try {
                ytPlayer.seekTo(GALAXY_SONG.start);
              } catch {
                /* ignore */
              }
            }
...
export const GalaxyLoveSection: React.FC = () => {
  ...
  const handleToggleAudio = useCallback(() => {
    const result = galaxyInstanceRef.current?.toggleAudio();
      ...
      <div
        className={`galaxy-canvas-wrapper relative mx-auto w-full transition-all duration-300 ${
          isFullscreen
            ...
            : "max-w-6xl h-[560px] sm:h-[660px] md:h-[760px] rounded-2xl sm:rounded-3xl border border-blue-500/20 shadow-2xl shadow-blue-950/60 overflow-hidden bg-black"
        }`}
      >
        <div
          id="galaxy-container"
          ref={mountRef}
          className="w-full h-full cursor-grab active:cursor-grabbing galaxy-canvas"
        />

        {/* Loading Hint - before intro starts */}
        {!hasStarted && (
          <div className="galaxy-intro-overlay absolute inset-0 z-10 pointer-events-none flex items-end justify-center pb-8 sm:pb-12">
            <div className="text-center px-6 py-3 rounded-2xl bg-black/30 backdrop-blur-md border border-white/10">
              <p className="text-white text-sm sm:text-base font-medium tracking-wide">
                Klik planetnya untuk memulai perjalanan ✨
              </p>
              <p className="text-slate-300 text-xs sm:text-sm mt-1">
                Putar suara agar lebih romantis
              </p>
            </div>
          </div>
        )}
        ...
        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 flex items-center gap-2">
          {hasStarted && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900/80 backdrop-blur-md border border-white/20 text-white text-xs hover:bg-slate-800 transition-all shadow-md active:scale-95 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={!audioReady}
            >
              {audioPlaying ? (
